...
use khal_std::glamx::{UVec2, UVec3, Vec2, Vec3, Vec3Swizzles, Vec4, Vec4Swizzles};
...
pub fn fdtd2_pml(
    #[spirv(global_invocation_id)] id3: UVec3,
    #[spirv(storage_buffer, descriptor_set = 0, binding = 0)] field_values: &mut [FieldValues2],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 1)] integrations: &mut [PmlIntegrations2],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 2)] update_coeffs: &[PmlCoefficients2],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 3)] source: &GpuSource2,
    #[spirv(storage_buffer, descriptor_set = 0, binding = 4)] source_vals: &[f32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 5)] step_counter: &mut u32,
    #[spirv(uniform, descriptor_set = 0, binding = 6)] grid: &GridInfo2,
) {
    let id2 = id3.xy();
    let grid_dim3 = UVec3::from((grid.grid_dim, 1));
    ...
    integrations[idx].e_curl += e_curl;
    field_values[idx].h = update_coeffs.h_coeffs[1] * field_values[idx].h +
        update_coeffs.h_coeffs[2] * e_curl +
        update_coeffs.h_coeffs[3] * integrations[idx].e_curl;
    ...
    let h_x_j1 = if is_boundary.y { 0. } else { field_values[idx_decremented.y].h.x };
    let h_curl = (h.y - h_y_i1) / d.x - (h.x - h_x_j1) / d.y;
    integrations[idx].dn_z += field_values[idx].dn_z;
    field_values[idx].dn_z = update_coeffs.dn_z_coeffs.dot(Vec3::new(
        field_values[idx].dn_z,
        h_curl,
        integrations[idx].dn_z
    ));
...
pub struct PmlCoefficients2 {
    pub h_coeffs: [Vec2; 4],
    pub dn_z_coeffs: Vec3,
    pub en_z_update_coeff: f32
}

#[derive(Copy, Clone, Pod, Zeroable, Default)]
#[repr(C)]
pub struct PmlIntegrations2 {
    pub e_curl: Vec2,
    pub dn_z: f32,
    pub _padding: u32,
}