...
pub fn fdtd1(
    ...
    #[spirv(storage_buffer, descriptor_set = 0, binding = 0)] cells: &mut [GridCell1],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 1)] materials: &mut [MaterialConstants1],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 2)] source_vals: &[f32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 3)] tfsf_source: &mut GpuSource1,
    ...
    #[spirv(uniform, descriptor_set = 0, binding = 6)] grid_info: &GridInfo1,
) {
    ...
    if src_cell_idx == idx {
        ...
        let dt_delay = (mat.n * grid_info.cell_size / MaterialConstants1::C_0 + grid_info.dt) / 2.;
...
#[spirv_bindgen]
#[spirv(compute(threads(64)))]
pub fn precompute_dft_kernels1(
    #[spirv(global_invocation_id)] id: UVec3,
    #[spirv(storage_buffer, descriptor_set = 0, binding = 0)] dft_kernels: &mut [GpuComplexPolar],
    #[spirv(uniform, descriptor_set = 0, binding = 1)] grid: &GridInfo1,
    #[spirv(uniform, descriptor_set = 0, binding = 2)] dft: &DftInfo1,
) {
    let i = id.x as usize;
...
pub fn dft1(
    ...
    #[spirv(storage_buffer, descriptor_set = 0, binding = 5)] cells: &[GridCell1],
...
pub fn finish_dft1(
    ...
    #[spirv(uniform, descriptor_set = 0, binding = 3)] grid: &GridInfo1,
) {
    let i = id.x as usize;
